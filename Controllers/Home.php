...
class Home extends Controller
{
    ...
    public function registrar(){
        if(empty($_POST['title']) || empty($_POST['start']) || empty($_POST['color'])){
            $mensaje = array('msg' => 'Todos los campos son necesarios.', 'estado' => false, 'tipo' => 'warning');
        } else {
            $evento = $_POST['title'];
            $fecha = $_POST['start'];
            $color = $_POST['color'];
            ...
            if($id == '') {
                $respuesta = $this->model->registrar($evento, $fecha, $color);
                    if($respuesta == 1) {
                        ...
                        $mensaje = array('msg' => 'Error al crear evento.', 'estado' => false, 'tipo' => 'error');
                    }
            } else {
                $respuesta = $this->model->modificar($evento, $fecha, $color, $id);
                    if($respuesta == 1) {
                        ...
                        $mensaje = array('msg' => 'Error al modificar evento.', 'estado' => false, 'tipo' => 'error');
                    }
            }
    ...
    public function eliminar($id){
        ...
        if($data == 1) {
            ...
            $mensaje = array('msg' => 'Error al eliminar evento.', 'estado' => false, 'tipo' => 'error');
        }
